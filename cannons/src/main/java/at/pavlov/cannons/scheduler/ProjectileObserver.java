...
public class ProjectileObserver {
    ...
    public void setupScheduler() {
        ...
        plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, () -> {
            //get projectiles
            Iterator<Map.Entry<UUID, FlyingProjectile>> iter = plugin
        ...
        for (Player p : loc.getWorld().getPlayers()) {
            ...
            if (distance <= maxDist) {
                plugin.getFakeBlockHandler().imitatedSphere(
                        ...
                        loc,
                        1,
                        Bukkit.createBlockData(liquid.getType()),
                        FakeBlockType.WATER_SPLASH,
                        1.0
                );
            }
    ...
    private void updateTeleporter(FlyingProjectile cannonball, org.bukkit.entity.Projectile projectile_entity) {
        ...
        if (projectile.hasProperty(ProjectileProperties.HUMAN_CANNONBALL) || projectile.hasProperty(ProjectileProperties.OBSERVER)) {
            ...
            Location optiLoc = projectile_entity.getLocation().clone().subtract(projectile_entity
                    .getVelocity()
                    .normalize()
                    .multiply(20.0));
    ...
    private void updateSmokeTrail(FlyingProjectile cannonball, org.bukkit.entity.Projectile projectile_entity) {
        ...
        if (proj.isSmokeTrailEnabled() && cannonball
                .getExpectedLocation()
                .distance(cannonball.getLastSmokeTrailLocation()) > smokeDist) {
            ...
            if (proj.isSmokeTrailParticleEnabled()) {
                cannonball.getWorld().spawnParticle(
                        proj.getSmokeTrailParticleType(),
                        newLoc,
                        proj.getSmokeTrailParticleCount(),
                        proj.getSmokeTrailParticleOffsetX(),
                        proj.getSmokeTrailParticleOffsetY(),
                        proj.getSmokeTrailParticleOffsetZ(),
                        proj.getSmokeTrailParticleSpeed(),
                        null,
                        true
                );
            } else {
                    ...
                    for (Player p : newLoc.getWorld().getPlayers()) {
                        ...
                        if (distance <= maxDist) {
                            plugin.getFakeBlockHandler().imitatedSphere(
                                    ...
                                    newLoc,
                                    0,
                                    proj.getSmokeTrailMaterial(),
                                    FakeBlockType.SMOKE_TRAIL,
                                    smokeDuration
                            );
                        }
