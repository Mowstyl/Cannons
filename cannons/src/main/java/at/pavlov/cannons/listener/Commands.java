            ...
            if (args.length >= 1) {
                    ...
                    if (args.length >= 2 && (
                        ...
                        if (offplayer != null && offplayer.hasPlayedBefore()) {
                            ...
                            if (b1) {
                                ...
                                sendMessage(
                                        sender,
                                        ChatColor.GREEN + userMessages
                                                .getMessage(MessageEnum.CannonsReseted)
                                                .replace("PLAYER", args[1])
                                );
                            } else {
                                sendMessage(
                                        sender,
                                        ChatColor.RED + "Player " + ChatColor.GOLD + args[1] + ChatColor.RED + " has no cannons."
                                );
                            }
                        } else {
                            sendMessage(
                        ...
                        sendMessage(
                                sender,
                                ChatColor.GREEN + "Missing player name " + ChatColor.GOLD + "'/cannons reset <NAME>' or '/cannons reset all' or '/cannons reset all_players'"
                        );
                    }
                    return true;
                }
                //cannons list
                    ...
                    if (args.length >= 2) {
                        ...
                        if (offplayer != null && offplayer.hasPlayedBefore()) {
                            sendMessage(
                                    sender,
                                    ChatColor.GREEN + "Cannon list for " + ChatColor.GOLD + offplayer.getName() + ChatColor.GREEN + ":"
                            );
                            for (Cannon cannon : CannonManager.getCannonList().values()) {
                                if (cannon.getOwner() != null && cannon.getOwner().equals(offplayer.getUniqueId())) {
                                    sendMessage(
                                            sender,
                                            ChatColor.GREEN + "Name:" + ChatColor.GOLD + cannon.getCannonName() + ChatColor.GREEN + " design:" + ChatColor.GOLD + cannon
                                                    .getCannonDesign()
                                                    .getDesignName() + ChatColor.GREEN + " location:" + ChatColor.GOLD + cannon
                                                    .getOffset()
                                                    .toString()
                                    );
                                }
                        ...
                        for (Cannon cannon : CannonManager.getCannonList().values()) {
                            if (cannon.getOwner() != null) {
                                ...
                                sendMessage(
                                        sender,
                                        ChatColor.GREEN + "Name:" + ChatColor.GOLD + cannon.getCannonName() + ChatColor.GREEN + " owner:" + ChatColor.GOLD + owner.getName() + ChatColor.GREEN + " location:" + ChatColor.GOLD + cannon
                                                .getOffset()
                                                .toString()
                                );
                            }
                    ...
                    if (player != null && player.hasPermission("cannons.admin.create")) {
                        if (args.length >= 2) {
                            ...
                            if (config.getDesignStorage().hasDesign(args[1])) {
                                ...
                                sendMessage(
                                        sender,
                                        ChatColor.RED + "[Cannons] Design not found Available designs are: " + String.join(
                                                ", ",
                                                plugin.getMyConfig().getDesignStorage().getDesignIds()
                                        )
                                );
                            }
                        } else {
                        ...
                        if (args.length >= 2) {
                            ...
                            if (projectile != null) {
                                ...
                                sendMessage(
                                        ...
                                        ChatColor.RED + "[Cannons] Design not found. Available designs are: " + String.join(
                                                ", ",
                                                ProjectileStorage.getProjectileIds()
                                        )
                                );
                            }
                        } else {
                    ...
                    if (player == null || player.hasPermission("cannons.admin.permissions")) {
                        ...
                        if (args.length >= 2 && args[1] != null) {
                            ...
                            if (permPlayer != null) {
                                ...
                                sendMessage(
                                        sender,
                                        ChatColor.GREEN + "Player not found. Usage: " + ChatColor.GOLD + "'/cannons permissions <NAME>'"
                                );
                            }
                        }
                        //the command sender is also a player - return the permissions of the sender
                            ...
                            sendMessage(
                                    sender,
                                    ChatColor.GREEN + "Missing player name " + ChatColor.GOLD + "'/cannons permissions <NAME>'"
                            );
                        }
                    } else {
                        plugin.logDebug("[Cannons] " + sender.getName() + " has no permission for command /cannons " + args[0]);
                ...
                //################### Player only commands #####################
                    ...
                    //add observer for cannon
                        ...
                        if (args.length >= 2 && (args[1].equalsIgnoreCase("off") || args[1].equalsIgnoreCase("disable") || args[1].equalsIgnoreCase(
                                "remove"))) {
                            ...
                            sendMessage(
                                    sender,
                                    ChatColor.RED + "Usage '/cannons observer' or '/cannons observer <off|disable>' or '/cannons observer <CANNON NAME>'"
                            );
                        }
                    }
                    ...
                    //toggle sentry target
                        ...
                        if (args.length >= 2 && (args[1].equalsIgnoreCase("mob"))) {
                            ...
                            sendMessage(
                                    sender,
                                    ChatColor.RED + "Usage '/cannons target <mob|player|cannon|other> <true|false> <range>'"
                            );
                        }
                    }
                    //get name of cannon
                    ...
                    //list cannons of this player name
                        ...
                        sendMessage(
                                sender,
                                ChatColor.GREEN + "Cannon list for " + ChatColor.GOLD + player.getName() + ChatColor.GREEN + ":"
                        );
                        for (Cannon cannon : CannonManager.getCannonList().values()) {
                            if (cannon.getOwner() != null && cannon.getOwner().equals(player.getUniqueId())) {
                                sendMessage(
                                        sender,
                                        ChatColor.GREEN + "Name:" + ChatColor.GOLD + cannon.getCannonName() + ChatColor.GREEN + " design:" +
                                                ChatColor.GOLD + cannon
                                                .getCannonDesign()
                                                .getDesignName() + ChatColor.GREEN + " loc: " + ChatColor.GOLD + cannon
                                                .getOffset()
                                                .toString()
                                );
                            }
                        ...
                        if (buildlimit < Integer.MAX_VALUE) {
                            ...
                            if (newcannons > 0) {
                                sendMessage(
                                        sender,
                                        ChatColor.GREEN + "You can build " + ChatColor.GOLD + newcannons + ChatColor.GREEN + " additional cannons"
                                );
                            } else {
                ...
                case BUY_CANNON -> {
                    if (cannon.isPaid()) {
                        ...
                        if (plugin.getEconomy() != null && cannon.getCannonDesign().getEconomyBuildingCost() > 0) {
                            EconomyResponse r = plugin.getEconomy().withdrawPlayer(
                                    player,
                                    cannon.getCannonDesign().getEconomyBuildingCost()
                            );
    ...
    public List<String> onTabComplete(CommandSender commandSender, Command cmd, String commandLabel, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("cannons")) {
            ...
            for (CommandList commandList : CommandList.values()) {
                if (commandList.getUsage().contains(full) && (commandList.getPermission() == null || commandSender.hasPermission(
                        commandList.getPermission()))) {
