...
public class CreateExplosion {
    ...
    private void spawnEntity(
            FlyingProjectile cannonball, Location loc, double entityVelocity,
        ...
        if (entity != null) {
            ...
            if (entity instanceof TNTPrimed tnt) {
                try {
                    int fusetime = CannonsUtil.parseInt(
                            entityHolder.getData().get(EntityDataType.FUSE_TIME),
                            tnt.getFuseTicks()
                    );
            ...
            if (entity instanceof AreaEffectCloud cloud) {
                try {
                    ...
                    cloud.setReapplicationDelay(
                            CannonsUtil.parseInt(
                                    entityHolder.getData().get(EntityDataType.REAPPLICATION_DELAY),
                                    cloud.getReapplicationDelay()
                            ));
                    cloud.setRadius(CannonsUtil.parseFloat(
                            entityHolder.getData().get(EntityDataType.RADIUS),
                            cloud.getRadius()
                    ));
                    ...
                    cloud.setDuration(CannonsUtil.parseInt(
                            entityHolder.getData().get(EntityDataType.DURATION),
                            cloud.getDuration()
                    ));
                    ...
                    cloud.setWaitTime(CannonsUtil.parseInt(
                            entityHolder.getData().get(EntityDataType.WAIT_TIME),
                            cloud.getWaitTime()
                    ));
                    cloud.setColor(
                    ...
                    cloud.setParticle(CannonsUtil.parseParticle(
                            entityHolder.getData().get(EntityDataType.PARTICLE),
                            cloud.getParticle()
                    ));
            ...
            if (entity instanceof SpectralArrow arrow) {
                try {
                    arrow.setGlowingTicks(CannonsUtil.parseInt(
                            entityHolder.getData().get(EntityDataType.DURATION),
                ...
                try {
                    ...
                    if (equipment != null) {
                        ...
                        equipment.setItemInMainHand(
                                (CannonsUtil.parseItemstack(
                                        entityHolder.getData().get(EntityDataType.MAIN_HAND_ITEM),
                                        equipment.getItemInMainHand()
                                )));
                        equipment.setItemInOffHand(
                                (CannonsUtil.parseItemstack(
                                        entityHolder.getData().get(EntityDataType.OFF_HAND_ITEM),
    ...
    public void detonate(FlyingProjectile cannonball, org.bukkit.entity.Projectile projectile_entity, BlockFace hitBlockFace) {
        ...
        ProjectileImpactEvent impactEvent = new ProjectileImpactEvent(projectile, impactLoc,
                cannonball.getShooterUID(), cannonball, hitBlockFace
        );     // CCNet - expose FlyingProjectile and hit BlockFace
        ...
        if (impactEvent.isCancelled()) {
            ...
            world.createExplosion(
                    ...
                    false,
                    false,
                    cannonball.getProjectileEntity()
            );
                        ...
                        new DelayedTask(cannonball) {
                            ...
                            public void run(Object object) {
                                ...
                                if (proj.isClusterExplosionsInBlocks() || expLoc.getBlock().isEmpty()
                                    ...
                                    expLoc.getWorld().createExplosion(
                                            expLoc,
                                            (float) proj.getClusterExplosionsPower(),
                                            projectile.hasProperty(ProjectileProperties.INCENDIARY),
                                            true,
                                            cannonball.getProjectileEntity()
                                    );
        ...
        if (impactLoc.getWorld() != null) {
            if (config.isImitatedExplosionParticlesEnabled()) {
                ...
                impactLoc.getWorld().spawnParticle(
                        config.getImitatedExplosionParticlesType(),
                        impactLoc,
                        config.getImitatedExplosionParticlesCount(),
                        d,
                        d,
                        d,
                        0,
