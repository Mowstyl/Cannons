...
public class Aiming {
    ...
    private void updateAimingMode() {
        ...
        while (iter.hasNext()) {
            ...
            if (System.currentTimeMillis() >= cannon.getLastAimed() + cannon.getCannonDesign().getAngleUpdateSpeed()) {
                ...
                if (playerInRange && player.isOnline() && cannon.isValid() && !(cannon
                        .getCannonDesign()
                        .isSentry() && cannon.isSentryAutomatic())) {
                    ...
                    if (cannon.getCannonDesign().isLinkCannonsEnabled()) {
                        ...
                        for (Cannon fcannon : CannonManager.getCannonsInBox(cannon.getLocation(), d, d, d)) {
                            ...
                            if (fcannon.isCannonOperator(player) && fcannon
                                    .getCannonDesign()
                                    .equals(cannon.getCannonDesign()) && (!cannon
                                    .getCannonDesign()
                                    .isAccessForOwnerOnly() || fcannon.getOwner() == player.getUniqueId())) {
    ...
    private void updateSentryMode() {
        ...
        while (iter.hasNext()) {
            ...
            if (cannon.isChunkLoaded() && System.currentTimeMillis() > (cannon.getLastSentryUpdate() + cannon
                    .getCannonDesign()
                    .getSentryUpdateTime())) {
                cannon.setLastSentryUpdate(System.currentTimeMillis());

                HashMap<UUID, Target> targets = CannonsUtil.getNearbyTargets(
                        cannon.getMuzzle(),
                        cannon.getCannonDesign().getSentryMinRange(),
                        cannon.getCannonDesign().getSentryMaxRange()
                );
                //old target - is this still valid?
                if (cannon.hasSentryEntity()) {
                    if (System.currentTimeMillis() > cannon.getSentryTargetingTime() + cannon
                            .getCannonDesign()
                            .getSentrySwapTime() || !targets.containsKey(cannon.getSentryEntity())) {
            ...
            // only update if since the last update some ticks have past (updateSpeed is in ticks = 50ms)
            if ((cannon.hasSentryEntity() || !cannon.isSentryHomedAfterFiring()) && System.currentTimeMillis() >= cannon.getLastAimed() + cannon
                    .getCannonDesign()
                    .getAngleUpdateSpeed()) {
                // autoaming or fineadjusting
                if (cannon.isValid()) {
...
//        }
    ...
    private boolean calculateTargetSolution(Cannon cannon, Target target, Vector targetVelocity, boolean addSpread) {
        ...
        if (cannon.getLoadedProjectile() != null && (cannon.getLoadedProjectile().getExplosionPower() > 2. || (cannon
                .getLoadedProjectile()
                .getPlayerDamage() > 1. && cannon.getLoadedProjectile().getPlayerDamageRange() > 2.))) {
        ...
        for (int i = 0; i < 100; i++) {
            Vector fvector = CannonsUtil.directionToVector(
                    cannon.getAimingYaw(),
                    cannon.getAimingPitch(),
                    cannon.getCannonballVelocity()
            );
            ...
            if (step < cannon.getCannonDesign().getAngleStepSize()) {
                if (verifyTargetSolution(cannon, target, 2.)) {
                    ...
                    if (addSpread) {
                        ...
                        cannon.setAimingPitch(cannon.getAimingPitch() + cannon
                                .getCannonDesign()
                                .getSentrySpread() * rand.nextGaussian());
                        cannon.setAimingYaw(cannon.getAimingYaw() + cannon
                                .getCannonDesign()
                                .getSentrySpread() * rand.nextGaussian());
                    }
    ...
    private boolean verifyTargetSolution(Cannon cannon, Target target, double maxdistance) {
        ...
        for (int i = 0; start.distance(predictor.getLoc()) < cannon
                .getCannonDesign()
                .getSentryMaxRange() * 1.2 && i < maxInterations; i++) {
            ...
            if (start.distance(predictor.getLoc()) > 1. && !block.isEmpty()) {
                ...
                return CannonsUtil
                        .findSurface(predictor.getLocation(), predictor.getVel())
                        .distance(target.getCenterLocation()) < maxdistance;
            }
    ...
    private double simulateShot(Vector vector, Location muzzle, Location target, EntityType projectileType, int maxInterations) {
        ...
        double target_distance = Math.sqrt(Math.pow(target.getX() - muzzle.getX(), 2) + Math.pow(target.getZ() - muzzle.getZ(),
                2));
        Vector oldLoc = null;
        for (int i = 0; i < 500; i++) {
            cannonball.updateProjectileLocation(false);
            Vector cLoc = cannonball.getLoc();
            if (Math.sqrt(Math.pow(cLoc.getX() - muzzle.getX(), 2) + Math.pow(cLoc.getZ() - muzzle.getZ(),
                    2)) > target_distance) {
    ...
    public void showAimingVector(Cannon cannon, Player player) {
        ...
        if (config.isImitatedAimingEnabled() && isImitatingEnabled(player.getUniqueId())) {
            plugin.getFakeBlockHandler().imitateLine(player,
                    cannon.getMuzzle(),
                    cannon.getAimingVector(),
                    0,
                    config.getImitatedAimingLineLength(),
                    config.getImitatedAimingMaterial(),
                    FakeBlockType.AIMING,
                    config.getImitatedAimingTime()
            );
        }
    }
    ...
    public Location impactPredictor(Cannon cannon) {
        if (cannon == null || cannon.getCannonballVelocity() < 0.01 || !config.isImitatedPredictorEnabled() || !cannon
                .getCannonDesign()
                .isPredictorEnabled()) {
            return null;
        }
            ...
            if (last.getValue() + design.getPredictorDelay() < System.currentTimeMillis()) {
                ...
                while (entry.hasNext()) {
                    ...
                    if (player != null && impact != null && plugin.getFakeBlockHandler().belowMaxLimit(player, impact)) {
                        plugin.getFakeBlockHandler().imitatedSphere(
                                player,
                                impact,
                                1,
