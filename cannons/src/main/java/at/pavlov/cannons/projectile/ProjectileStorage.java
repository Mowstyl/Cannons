...
public class ProjectileStorage {
    ...
    public static Projectile getProjectile(Cannon cannon, ItemHolder materialHolder) {
        for (Projectile projectile : projectileList) {
            if (cannon.getCannonDesign().canLoad(projectile) && !projectile
                    .getLoadingItem()
                    .equals(Material.AIR) && projectile.equals(materialHolder)) {
                return projectile;
            }
        }
        return null;
    ...
    private Projectile loadYml(String ymlFile) {
        ...
        projectile.setCharges(projectileConfig.getInt("cannonball.numberOfCharges", 1)); // CCNet
        projectile.setImpactDurabilityDamage(projectileConfig.getInt("cannonball.impactDurabilityDamage")); // CCNet
        projectile.setDeflectionFactor(projectileConfig.getDouble("cannonball.deflectionFactor", 0)); // CCNet
        ...
        projectile.setSmokeTrailMaterial(Bukkit.createBlockData(projectileConfig.getString(
                "smokeTrail.material",
                "minecraft:cobweb"
        )));
        ...
        projectile.setSmokeTrailParticleType(Particle.valueOf(projectileConfig.getString(
                "smokeTrail.particles.type",
        ...
        projectile.setExplosionPowerDependsOnVelocity(projectileConfig.getBoolean(
                "explosion.explosionPowerDependsOnVelocity",
                true
        ));
        ...
        projectile.setSoundImpactProtected(new SoundHolder(projectileConfig.getString(
                "sounds.impactProtected",
                "ENTITY_GENERIC_EXPLODE:10:0.5"
        )));
        projectile.setSoundImpactWater(new SoundHolder(projectileConfig.getString(
                "sounds.impactWater",
                "ENTITY_GENERIC_SPLASH:10:0.3"
        )));
